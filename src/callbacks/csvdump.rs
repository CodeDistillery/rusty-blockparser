...
impl Callback for CsvDump {
    ...
    fn on_block(&mut self, block: Block, _block_height: usize) {
        // serialize block
        // self.block_writer.write_all(block.as_csv(_block_height).as_bytes()).unwrap();
...
impl EvaluatedTxOut {
    #[inline]
    fn as_csv(&self, _txid: &str, _index: usize) -> String {
